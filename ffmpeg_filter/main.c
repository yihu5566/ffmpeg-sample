            ...
            if (avcodec_send_packet(avctx, pkt) == AVERROR(EAGAIN)) {
                ...
                for (;;) {
                    ...
                    if (AVERROR(EAGAIN) == ret) {
                        ...
                        if (!swr_ctx) {
                            swr_ctx = swr_alloc_set_opts(NULL, frame->channel_layout, tgt_fmt, tgt_freq,
                                                         frame->channel_layout, frame->format, frame->sample_rate, 0,
                                                         NULL);
