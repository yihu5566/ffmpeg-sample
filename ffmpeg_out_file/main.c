...
int main() {
    ...
    //打开输出文件容器
    char *filename_out = "/Users/dongfang/CLionProjects/ffmpeg-sample/mp4_file/juren-30s-5.mp4";
            ...
            if (AVERROR(EAGAIN) == ret) {
                ...
                * */
                ...
                for (;;) {
                    ...
                    pkt_out->pts = av_rescale_q_rnd(pkt_out->pts, fmt_ctx->streams[0]->time_base, st->time_base,
                                                    AV_ROUND_NEAR_INF | AV_ROUND_PASS_MINMAX);
                    pkt_out->dts = av_rescale_q_rnd(pkt_out->dts, fmt_ctx->streams[0]->time_base, st->time_base,
                                                    AV_ROUND_NEAR_INF | AV_ROUND_PASS_MINMAX);
                ...
                for (;;) {
                    ...
                    if (ret < 0) {
                        printf("avcodec_receive_packet fial %d \n", ret);
                        return ret;
                    }
                    //编码出 AVPacket ，先打印一些信息，然后把它写入文件。
                    printf("pkt_out size : %d \n", pkt_out->size);
